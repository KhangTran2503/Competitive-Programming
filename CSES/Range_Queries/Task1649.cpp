...
struct SegTree{
	private:
		...
		void __build(int id,int l,int r){
			...
			this->seg[id] = min(this->seg[id << 1],this->seg[(id << 1) | 1]);
		}
		...
		T __get(int u,int v,int id = 1){
			if (this->L[id] > v || this->R[id] < u) return INF;
			if (u <= L[id] && R[id] <= v) return seg[id];
			return min(__get(u,v,id << 1),__get(u,v,(id << 1) | 1));
		}
	...
	public:
		SegTree(vector<int> a){
			...
			this->seg.assign(4*this->size + 5,INF);

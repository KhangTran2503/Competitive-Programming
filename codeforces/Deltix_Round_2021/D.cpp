...
int main(){
	...
	while (1){
		...
		vector<int> dp(1 << n_bit,0);
		//dp[mask] la so luong cac submask chua mask 
		...
		//for (int mask = 1; mask < (1 << n_bit); mask++) cout << dp[mask] << ' ';
		//cout << '\n';
		
		int max_mask = (1 << n_bit);
		for (int j = 0; j < n_bit; j++)
			for (int msk = 0; msk < max_mask; msk++)
				if ((msk >> j) & 1) dp[msk ^ (1 << j)] += dp[msk];
		
		
		
		for (int msk = 0; msk < max_mask; msk++)
			if (2*dp[msk] >= n && __builtin_popcount(msk) > max_nbit){
